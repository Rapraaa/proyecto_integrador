import { Test, TestingModule } from '@nestjs/testing';
import { TechCategoriesController } from './tech-categories.controller';
import { TechCategoriesService } from '../services/tech-categories.service';

describe('TechCategoriesController', () => {
  let controller: TechCategoriesController;

  const mockService = {
    create: jest.fn(),
    findAll: jest.fn(),
    findOne: jest.fn(),
    update: jest.fn(),
    remove: jest.fn(),
  };

  beforeEach(async () => {
    jest.clearAllMocks();
    const module: TestingModule = await Test.createTestingModule({
      controllers: [TechCategoriesController],
      providers: [
        { provide: TechCategoriesService, useValue: mockService },
      ],
    }).compile();
    controller = module.get<TechCategoriesController>(TechCategoriesController);
  });

  it('debe estar definido', () => {
    expect(controller).toBeDefined();
  });

  it('findAll delega la consulta al servicio', async () => {
    const query = { page: 1, limit: 10 };
    mockService.findAll.mockResolvedValue({ data: [], meta: {} });
    await controller.findAll(query);
    expect(mockService.findAll).toHaveBeenCalledWith(query);
  });

  it('findOne delega el id al servicio', async () => {
    mockService.findOne.mockResolvedValue({ id: 'tc-1' });
    await controller.findOne('tc-1');
    expect(mockService.findOne).toHaveBeenCalledWith('tc-1');
  });

  it('create delega el dto al servicio', async () => {
    const dto = { name: 'language' };
    mockService.create.mockResolvedValue({ id: 'tc-1', ...dto });
    await controller.create(dto);
    expect(mockService.create).toHaveBeenCalledWith(dto);
  });

  it('update delega id y dto al servicio', async () => {
    const dto = { isActive: false };
    mockService.update.mockResolvedValue({ id: 'tc-1', ...dto });
    await controller.update('tc-1', dto);
    expect(mockService.update).toHaveBeenCalledWith('tc-1', dto);
  });

  it('remove delega el id al servicio', async () => {
    mockService.remove.mockResolvedValue({ id: 'tc-1' });
    await controller.remove('tc-1');
    expect(mockService.remove).toHaveBeenCalledWith('tc-1');
  });
});
